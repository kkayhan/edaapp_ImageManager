...
Upload the SR‑SIM `.zip` exactly like the others — it's auto‑detected. The row shows **Ready** as soon as the image is unpacked and served; click **Details** for a complete sim **NodeProfile** (its `containerImage` points at this app). The node can pull it out of the box — the app configures the node's containerd automatically; see [how that works](#sr-sim--sr-os-simulator-for-the-digital-twin) below (Talos is the one exception).

**How the node pulls from the app — handled for you.** Container images are pulled by the node's container runtime (containerd), which by default can't resolve the app's in‑cluster Service name or trust its serving cert. Image Manager ships a small **node‑agent DaemonSet** (`eda-imagemanager-node-agent`, in `eda-system`) that fixes this on every node automatically: it writes a containerd **registry‑hosts redirect** (`/etc/containerd/certs.d/eda-imagemanager.eda-system.svc/hosts.toml`) pointing the registry FQDN at the app's **live Service ClusterIP**, with `skip_verify` on that endpoint (the serving cert's SAN is the FQDN, not the ClusterIP, so verifying against the IP would fail — image **layers are still digest‑verified**, so integrity is unaffected). It re‑resolves the ClusterIP every cycle, so it **self‑heals** if the Service is recreated. This is the same `config_path` mechanism EDA's own `edabuilder deploy` uses; containerd honours it with no restart. Nothing to do on standard containerd clusters (kubeadm, Kind, k3s/RKE2).

**Exception — Talos (immutable nodes).** Talos rebuilds `/etc` from machine config on boot and doesn't expose the containerd `config_path` hosts directory, so the DaemonSet can't durably configure it (it detects this and logs guidance instead of writing a dead file). On Talos, add the equivalent **registry mirror** to the machine config once per cluster — map the app's Service name to its ClusterIP and skip‑verify the in‑cluster serving cert **on the endpoint** (it has no IP SAN):
