...
IPALLOCATIONPOOL_SCHEMA = eda.Schema(group='core.eda.nokia.com', version='v1', kind='IPAllocationPool')


class IPAllocationPoolIpSpec:
    def __init__(
        self,
        subnet: str,
    ...
    @staticmethod
    def from_input(obj) -> 'IPAllocationPoolIpSpec | None':
        if obj:
...
class IPAllocationPoolSpec:
    def __init__(
        self,
        segments: list[IPAllocationPoolIpSpec] | None = None,
    ...
    @staticmethod
    def from_input(obj) -> 'IPAllocationPoolSpec | None':
        if obj:
            _segments = []
            if obj.get(Y_SEGMENTS) is not None:
                for x in obj.get(Y_SEGMENTS):
                    _segments.append(IPAllocationPoolIpSpec.from_input(x))
            _publishAllocations = obj.get(Y_PUBLISHALLOCATIONS)
            return IPAllocationPoolSpec(
                segments=_segments,
...
class IPAllocationPoolStatus:
    ...
    @staticmethod
    def from_input(obj) -> 'IPAllocationPoolStatus | None':
        if obj:
            return IPAllocationPoolStatus(
            )
        return None  # pragma: no cover
...
class IPAllocationPool:
    def __init__(
        ...
        spec: IPAllocationPoolSpec | None = None,
        status: IPAllocationPoolStatus | None = None
    ):
    ...
    def to_input(self):  # pragma: no cover
        _rval = {}
        _rval[Y_SCHEMA_KEY] = IPALLOCATIONPOOL_SCHEMA
    ...
    @staticmethod
    def from_input(obj) -> 'IPAllocationPool | None':
        if obj:
            _metadata = (
            ...
            _spec = IPAllocationPoolSpec.from_input(obj.get(Y_SPEC, None))
            _status = IPAllocationPoolStatus.from_input(obj.get(Y_STATUS))
            return IPAllocationPool(
                metadata=_metadata,
                spec=_spec,
                status=_status,
            )
...
class IPAllocationPoolList:
    def __init__(
        self,
        items: list[IPAllocationPool],
    ...
    @staticmethod
    def from_input(obj) -> 'IPAllocationPoolList | None':
        if obj:
            _items = obj.get(Y_ITEMS, [])
            _listMeta = obj.get(Y_METADATA, None)
            return IPAllocationPoolList(
                items=_items,
                listMeta=_listMeta,
            )
        return None  # pragma: no cover
